...
function [Y, R, E] = Isomap(D, n_fcn, n_size, options,labels,N,data); 
...
if (displ==1)
     ...
     if ~isempty(twod)
		...
		for z=1:datasize
			if(mod(z, 40)==0)
				img = reshape(data(:,z), 28, 28);
				img = img';
				RgbImage = repmat(uint8(255.*img), [1 1 3]);
				imagesc([Y.coords{twod}(1,z)+0.5 Y.coords{twod}(1,z)+5*power(10,2)], [Y.coords{twod}(2,z)+5*power(10, 2) Y.coords{twod}(2,z)+0.5], RgbImage);
			end
		    plot(Y.coords{twod}(1, find(labels==1)), Y.coords{twod}(2,find(labels==1)), 'ro');
		    plot(Y.coords{twod}(1, find(labels==7)), Y.coords{twod}(2,find(labels==7)), 'bo');
		    legend('1', '7');
       end
         if (overlay == 1)
